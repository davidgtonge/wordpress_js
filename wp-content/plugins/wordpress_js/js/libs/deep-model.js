                    ...
                    if (val.constructor === Object) {
                        ...
                        if (!_.isEqual(getNested(now, attr), val)) {
                            ...
                            if (!options.silent) {
                                ...
                                var fields, key, _i, _len, last, change_events;
                                fields = attr.split('.');
                                change_events = [];
                                if(fields.length > 1){
                                    for (_i = 0, _len = fields.length, last = _len - 1; _i < _len; _i++) {
                                      key = fields[_i];
                                      change_events.push('change:' + key);

                                    }
                                    _(change_events).chain().uniq().each(function(event){
                                        //self.trigger(event, self, val, fields[last]);
                                    });
                                }
